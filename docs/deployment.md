...
| `MODELHARBOR_PUBLIC_BASE_URL` | `http://localhost:${PORT}` | 供外部访问的公网 URL,用于在管理后台(Overview / Settings → 公共端点)展示可复制的完整 endpoint URL。反向代理场景下应填 `https://llm.example.com` 这类外部可达地址。 |
