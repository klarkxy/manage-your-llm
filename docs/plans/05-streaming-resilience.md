...
- [x] 1. 实现 stream adapter contract。
- [x] 2. 实现 Anthropic SSE。
- [x] 3. 实现 OpenAI Chat SSE。
- [x] 4. 实现 Responses SSE。
- [x] 5. 实现 first-token timeout。
- [x] 6. 实现 stream usage parser。
- [x] 7. 实现 conversation sticky。
- [x] 8. 实现 session sticky。
- [x] 9. 实现 circuit breaker。
- [x] 10. 实现 cooldown。（upstream 级 cooldown 写入 + 过期清理）
- [x] 11. 实现 endpoint health worker。
- [x] 12. 实现 manual model ping（同时写入 endpoint health）。
- [x] 13. 实现 maintenance service。
