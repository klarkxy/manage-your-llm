...
4.（可选）点击 **Fetch models** 从供应商的 `/v1/models` 端点自动发现模型列表。
5. 填写模型映射。每一行包含：
   - **Real name**：供应商处的真实模型 ID。
   - **Public name**：展示给下游的别名（可选，留空则与 real name 相同）。
   - **Enabled**：是否启用该映射。
6. 设置配额（可选）：
...
| 操作 | 说明 |
| --- | --- |
| **Freeze** | 手动冻结某条上游密钥，路由时不再选择它。 |
| **Unfreeze** | 手动解除冻结。配额冻结需等周期重置或手动解除。 |
| **Rotate secret** | 更换上游 API key。旧 key 失效，新 key 立即生效。 |
| **Ping** | 在弹窗中测试某个候选模型是否可达，显示延迟和状态。 |
| **Health** | 查看每个 endpoint 的探测延迟和是否降级。 |
