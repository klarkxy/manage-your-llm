...
//
// - Reads comma-separated ports from MODELHARBOR_PORTS (default: 5420,5421,
...
const DEFAULT_WEB_PORT = process.env.MODELHARBOR_WEB_PORT ?? '5421';
const PORTS = (process.env.MODELHARBOR_PORTS ?? `5420,${DEFAULT_WEB_PORT}`)
  .split(',')
  .map((s) => Number(s.trim()))
