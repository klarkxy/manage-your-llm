...
describe("gateway auth", () => {
  ...
  it("rejects /v1/messages without a consumer key", async () => {
    const res = await rig.app.inject({ method: "POST", url: "/v1/messages", payload: ANTHROPIC_BODY });
  ...
  it("rejects /v1/chat/completions without a consumer key", async () => {
    const res = await rig.app.inject({ method: "POST", url: "/v1/chat/completions", payload: OPENAI_BODY });
    expect(res.statusCode).toBe(401);
  });
...
describe("gateway happy path: /v1/chat/completions via openai_compatible upstream", () => {
  ...
  it("returns an OpenAI-shaped non-stream response", async () => {
    ...
    expect(rig.fake.openaiRequests[0]!.headers["authorization"]).toBe(`Bearer ${rig.rawUpstreamKey}`);
  });
});
...
describe("gateway candidate filtering", () => {
  ...
  it("does not route an anthropic request to an openai_compatible upstream", async () => {
  ...
  it("does not route an openai request to an anthropic_compatible upstream", async () => {
...
describe("gateway priority policy", () => {
  it("selects the lower-priority candidate when the higher one is filtered out", async () => {
    ...
    try {
      ...
      const healthyEnc = (await import("../src/modules/auth/crypto.js")).encryptSecret("sk-healthy-key-1", rig.secretKey);
      ...
      await rig.db.update(upstreamKeys).set({ frozen: true, frozenReason: "test" }).where(eq(upstreamKeys.id, rig.upstreamKeyId));
      ...
      expect(res.statusCode).toBe(200);
      expect((res.json() as { content: Array<{ text: string }> }).content[0]?.text).toBe("from healthy");
    } finally {
      await rig.close();
    }
  });
...
describe("gateway failover with cooldown", () => {
  it("cools down a quota-exhausted upstream and falls back to the next candidate", async () => {
    ...
    try {
      ...
      expect(res.statusCode).toBe(200);
      expect((res.json() as { content: Array<{ text: string }> }).content[0]?.text).toBe("from backup");
...
describe("GET /v1/models", () => {
  it("returns only the public model and group the consumer key can access", async () => {
    ...
    try {
      ...
      expect(res.statusCode).toBe(200);
      const body = res.json() as { object: string; data: Array<{ id: string; metadata: { target_type: string } }> };
