...
describe('admin setup routes', () => {
  ...
  it('returns setup status with admin already bootstrapped', async () => {
  ...
  it('verifies security credentials', async () => {
  ...
  it('creates first upstream and updates setup status', async () => {
    ...
    expect(res.statusCode).toBe(200);
    const body = JSON.parse(res.payload);
    expect(body.data.upstreamKeyId).toMatch(/^uk_/);
  ...
  it('creates public models and default consumer key through setup', async () => {
    ...
    const upstreamKeyId = JSON.parse(upstream.payload).data.upstreamKeyId;
    ...
    const models = await app.inject({
      ...
      payload: {
        models: [
          {
            name: 'gpt-4o',
            candidates: [{ upstreamKeyId, realModelName: 'gpt-4o', priority: 100 }],
          },
        ],
      },
    });
    ...
    expect(modelsBody.data.publicModelIds).toHaveLength(1);

    const key = await app.inject({
      method: 'POST',
      url: '/api/admin/setup/consumer-key',
