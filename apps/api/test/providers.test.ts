...
describe("anthropic-compatible adapter", () => {
  ...
  it("omits optional fields when not set", () => {
    const ctx = makeContext({
      ir: { ...makeContext().ir, system: null, maxTokens: null, temperature: null, topP: null, metadata: {} },
    });
    const req = buildAnthropicCompatibleRequest({ ...ctx, baseUrl: "https://api.example.com", apiKey: "k" });
  ...
  it("classifies 401 as provider_authentication", () => {
    ...
    const err = adapter.normalizeError({ response: resp, request: makeContext(), transportError: undefined });
...
describe("end-to-end through fake upstream", () => {
  ...
  it("round-trips an Anthropic request and response", async () => {
    const ctx = makeContext();
    const built = buildAnthropicCompatibleRequest({ ...ctx, baseUrl: rig.baseUrl, apiKey: rig.apiKey });
  ...
  it("round-trips an OpenAI request and response", async () => {
    ...
    const built = buildOpenAICompatibleRequest({ ...ctx, baseUrl: rig.baseUrl, apiKey: rig.apiKey });
  ...
  it("normalizes an upstream 401 as provider_authentication over the wire", async () => {
    ...
    const ctx = makeContext();
    const built = buildAnthropicCompatibleRequest({ ...ctx, baseUrl: rig.baseUrl, apiKey: "sk-wrong" });
