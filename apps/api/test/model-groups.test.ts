...
describe("model groups admin", () => {
  ...
  it("rolls back the whole create when a member points at a missing public model", async () => {
    ...
    const mg = await rig.db.select().from(modelGroups).where(eq(modelGroups.name, "mg-rollback")).get();
    expect(mg).toBeUndefined();
    const tn = await rig.db.select().from(targetNames).where(eq(targetNames.name, "mg-rollback")).get();
