...
describe("transactional replace semantics", () => {
  ...
  it("PUT public-model candidates rolls back when the new batch is invalid", async () => {
    ...
    const before = await rig.db.select().from(publicModelCandidates).where(eq(publicModelCandidates.publicModelId, pmId)).all();
    ...
    const after = await rig.db.select().from(publicModelCandidates).where(eq(publicModelCandidates.publicModelId, pmId)).all();
  ...
  it("insertTargetRow rolls back the target row when the target_names insert fails", async () => {
    ...
    const pm = await rig.db.select().from(publicModels).where(eq(publicModels.name, "race-pm")).get();
  ...
  it("deleteTargetRow rolls back the target_names row when the target delete fails", async () => {
    ...
    await rig.db.delete(modelGroupMembers).where(eq(modelGroupMembers.modelGroupId, refs.modelGroupId));
    ...
    const grp = await rig.db.select().from(modelGroups).where(eq(modelGroups.id, refs.modelGroupId)).get();
    expect(grp).toBeUndefined();
    const tn = await rig.db.select().from(targetNames).where(eq(targetNames.targetId, refs.modelGroupId)).get();
