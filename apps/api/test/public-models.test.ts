...
import { publicModels, targetNames, publicModelCandidates, upstreamKeys } from "../src/modules/db/index.js";
...
describe("public models admin", () => {
  ...
  it("creates a public model with candidates and a target_names row", async () => {
    ...
    const body = res.json() as { name: string; candidates: { realModelName: string }[]; candidateCount: number };
  ...
  it("PUT candidates replaces the candidate list transactionally", async () => {
    ...
    const candidates = await rig.db.select().from(publicModelCandidates).where(eq(publicModelCandidates.publicModelId, id)).all();
  ...
  it("deletes a public model and its target_names row together", async () => {
    ...
    const pm = await rig.db.select().from(publicModels).where(eq(publicModels.id, refs.publicModelId)).get();
    expect(pm).toBeUndefined();
    const tn = await rig.db.select().from(targetNames).where(eq(targetNames.name, "ds-v4-flash")).get();
    expect(tn).toBeUndefined();
    // The seeded upstream key must still be present.
    const uk = await rig.db.select().from(upstreamKeys).where(eq(upstreamKeys.id, refs.upstreamKeyId)).get();
  ...
  it("rolls back the whole create when a candidate points at a missing upstream key", async () => {
    ...
    const pm = await rig.db.select().from(publicModels).where(eq(publicModels.name, "ds-rollback")).get();
    expect(pm).toBeUndefined();
    const tn = await rig.db.select().from(targetNames).where(eq(targetNames.name, "ds-rollback")).get();
    expect(tn).toBeUndefined();
