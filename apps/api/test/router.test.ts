...
import {
  ...
  filterCandidates,
} from "../src/modules/router/candidates.js";
...
describe("filterCandidates", () => {
  ...
  it("drops a candidate whose provider type does not match the source protocol", () => {
    const res = filterCandidates(
      [candidate({ providerType: "openai_compatible" })],
      { sourceProtocol: "anthropic", now: new Date() },
    );
    expect(res.accepted).toHaveLength(0);
    expect(res.dropped[0]?.reason).toBe("protocol_mismatch");
  });
});
