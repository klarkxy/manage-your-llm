...
describe('createEnv', () => {
  ...
  it('rejects default ADMIN_PASSWORD in production', () => {
    ...
    expect(() => createEnv()).toThrow(/ADMIN_PASSWORD/);
  });

  it('rejects default ADMIN_USERNAME in production', () => {
    process.env['NODE_ENV'] = 'production';
    process.env['MYLLM_SECRET_KEY'] = 'not-default';
    process.env['MYLLM_ADMIN_PASSWORD'] = 'not-default';
    expect(() => createEnv()).toThrow(/ADMIN_USERNAME/);
  });

  it('accepts custom credentials in production', () => {
    process.env['NODE_ENV'] = 'production';
    process.env['MYLLM_SECRET_KEY'] = 'not-default';
    process.env['MYLLM_ADMIN_USERNAME'] = 'administrator';
    process.env['MYLLM_ADMIN_PASSWORD'] = 'super-secret';
    const env = createEnv();
    expect(env.SECRET_KEY).toBe('not-default');
    expect(env.ADMIN_USERNAME).toBe('administrator');
    expect(env.ADMIN_PASSWORD).toBe('super-secret');
  });
