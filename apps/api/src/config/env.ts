...
const DEFAULTS: Env = {
  ...
  DATABASE_URL: 'file:./data/manageyourllm.sqlite',
...
function readEnv(): Env {
  ...
  return {
    ...
    ADMIN_USERNAME: envString(
      'MYLLM_ADMIN_USERNAME',
      envString('MANAGE_YOUR_LLM_ADMIN_USERNAME', DEFAULTS.ADMIN_USERNAME),
    ),
    ADMIN_PASSWORD: envString(
      'MYLLM_ADMIN_PASSWORD',
      envString('MANAGE_YOUR_LLM_ADMIN_PASSWORD', DEFAULTS.ADMIN_PASSWORD),
    ),
    ADMIN_DISPLAY_NAME: envString(
      'MYLLM_ADMIN_DISPLAY_NAME',
      envString('MANAGE_YOUR_LLM_ADMIN_DISPLAY_NAME', DEFAULTS.ADMIN_DISPLAY_NAME),
    ),
...
export function createEnv(): Env {
  ...
  if (cached.NODE_ENV === 'production') {
    ...
    if (cached.ADMIN_PASSWORD === DEFAULTS.ADMIN_PASSWORD) {
      throw new Error(
        'MYLLM_ADMIN_PASSWORD / MANAGE_YOUR_LLM_ADMIN_PASSWORD must be set in production',
      );
    }
    if (cached.ADMIN_USERNAME === DEFAULTS.ADMIN_USERNAME) {
      throw new Error(
        'MYLLM_ADMIN_USERNAME / MANAGE_YOUR_LLM_ADMIN_USERNAME must be set in production',
      );
    }
  }
  return cached;
