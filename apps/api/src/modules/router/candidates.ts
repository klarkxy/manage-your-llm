...
export async function expandCandidates(
  ...
  if (args.targetType === "public_model") {
    ...
    return rows.map(toResolvedCandidate);
  }
...
// Filter the candidate list for a specific request. Drops:
//   - disabled candidate rows
//   - disabled public models
//   - disabled upstream keys
//   - frozen upstream keys
//   - upstream keys currently in cooldown
//   - upstream keys whose provider type does not speak the source protocol
//     (MVP does not do cross-protocol routing)
//   - upstream keys that are over quota (M6). The caller passes in the set of
//     upstreamKeyIds that are currently over quota; the candidates module does
//     not talk to the quota engine directly.
export function filterCandidates(
  ...
  for (const c of candidates) {
    ...
    if (protocolFor(c.providerType) !== args.sourceProtocol) {
      dropped.push({ candidate: c, reason: "protocol_mismatch" });
      continue;
    }
    accepted.push(c);
  }
