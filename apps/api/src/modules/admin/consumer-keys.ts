...
import {
  ...
  apps,
  consumerKeys,
  consumerKeyAccess,} from "../db/index.js";
import {
...
export function registerConsumerKeyRoutes(app: FastifyInstance, deps: ConsumerKeyRouteDeps): void {
  ...
  app.get("/api/admin/apps/:appId/consumer-keys", async (req, reply) => {
    ...
    if (!app) {
      reply.code(404).send({ error: { message: "app not found", type: "target_not_found", code: "target_not_found" } });
      return;
    }
    const rows = await db
      .select()
  ...
  app.post("/api/admin/consumer-keys/:id/revoke", async (req, reply) => {
    ...
    if (!existing) {
      reply.code(404).send({ error: { message: "consumer key not found", type: "target_not_found", code: "target_not_found" } });
      return;
    }
    await db
      .update(consumerKeys)
  ...
  app.post("/api/admin/consumer-keys/:id/rotate", async (req, reply) => {
    ...
    await db
      .update(consumerKeys)
      .set({ keyHash: generated.hash, keyPrefix: generated.prefix, enabled: true, revokedAt: null, updatedAt: new Date() })
      .where(eq(consumerKeys.id, id));
    const row = await db.select().from(consumerKeys).where(eq(consumerKeys.id, id)).get();
