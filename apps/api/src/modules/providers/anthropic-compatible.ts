...
export function createAnthropicCompatibleAdapter(): ProviderAdapter {
  return {
    ...
    buildRequest(context: ProviderRequestContext): ProviderHttpRequest {
      ...
      return {
        method: "POST",
        url: `${base}${ANTHROPIC_MESSAGES_PATH}`,
        headers: {
    ...
    normalizeStreamEvent(context: ProviderStreamEventContext): ProviderStreamEventResult {
      ...
      if (event === "message_start") {
        ...
        let parsed: { message?: { usage?: { input_tokens?: number; output_tokens?: number } } } | null = null;
        try {
          parsed = JSON.parse(data) as { message?: { usage?: { input_tokens?: number; output_tokens?: number } } };
        } catch {
          return { kind: "open" };
        }
      ...
      if (event === "message_delta") {
        let parsed: { delta?: { stop_reason?: string | null }; usage?: { input_tokens?: number; output_tokens?: number; cache_creation_input_tokens?: number; cache_read_input_tokens?: number } } | null = null;
        try {
          parsed = JSON.parse(data) as { delta?: { stop_reason?: string | null }; usage?: { input_tokens?: number; output_tokens?: number; cache_creation_input_tokens?: number; cache_read_input_tokens?: number } };
        } catch {
...
export function buildAnthropicCompatibleRequest(context: ProviderRequestContext): ProviderHttpRequest {
