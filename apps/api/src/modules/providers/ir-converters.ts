...
export function anthropicRequestToIR(
  ...
  if (body.metadata && typeof body.metadata === "object" && typeof (body.metadata as { user_id?: unknown }).user_id === "string") {
...
export function openaiRequestToIR(
  ...
  for (const m of body.messages ?? []) {
    ...
    if (m.role === "tool") {
      ...
      messages.push({ role: "tool", content: extractOpenAIText(m.content), ...(id ? { toolCallId: id } : {}) });
      continue;
    }
