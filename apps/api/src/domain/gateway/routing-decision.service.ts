import { requiredCapabilities, type RequiredCapabilities, type SourceProtocol } from '@manageyourllm/shared';
...
import type { RoutingDecision, RoutingDecisionInput, RoutingCandidate, TraceEvent } from './routing.types.js';
import type { UpstreamKeyRow, PublicModelCandidateRow, ModelGroupMemberRow, AdminSettingsRow, QuotaPeriod } from '../../infrastructure/db/schema.js';
...
function providerSupportsCapability(providerType: UpstreamKeyRow['providerType'], capability: keyof RequiredCapabilities): boolean {
...
function providerSupportsProtocol(providerType: UpstreamKeyRow['providerType'], sourceProtocol: SourceProtocol): boolean {
...
export class RoutingDecisionService {
  ...
  async decide(input: RoutingDecisionInput): Promise<RoutingDecision> {
    ...
    events.push({ step: 'candidates_expand', status: 'info', details: { targetType: resolvedTarget.type, targetName: resolvedTarget.name } });
    ...
    events.push({ step: 'candidates_filter', status: 'info', details: { before: candidates.length } });
    ...
    if (settings.enableStickySession) {
      ...
      if (binding) {
        const matchIndex = finalCandidates.findIndex(
          (c) => c.upstreamKey.id === binding.upstreamKeyId && c.realModelName === binding.realModelName,
        );
        if (matchIndex > 0) {
          ...
          if (match) {
            ...
            stickyHit = true;
            events.push({ step: 'sticky_binding_hit', status: 'ok', details: { upstreamKeyId: binding.upstreamKeyId } });
          }
      ...
      if (!stickyHit) {
        const session = await this.routingStateRepo.findStickySession(consumerKeyId, ir.requestedModel, now);
        if (session) {
          const matchIndex = finalCandidates.findIndex(
            (c) => c.upstreamKey.id === session.upstreamKeyId && c.realModelName === session.realModelName,
          );
          if (matchIndex > 0) {
            const match = finalCandidates.splice(matchIndex, 1)[0];
            if (match) {
              ...
              events.push({ step: 'sticky_session_hit', status: 'ok', details: { upstreamKeyId: session.upstreamKeyId } });
            }
          } else if (matchIndex === 0) {
            sessionStickyHit = true;
          }
    ...
    events.push({ step: 'routing_decision', status: 'ok', details: { after: finalCandidates.length } });
  ...
  private async expandPublicModel(publicModelId: string, _events: TraceEvent[]): Promise<RoutingCandidate[]> {
  ...
  private async expandModelGroup(groupId: string, events: TraceEvent[]): Promise<RoutingCandidate[]> {
    ...
    if (policy === 'weighted') {
      ...
      events.push({ step: 'group_weighted_pick', status: 'ok', details: { selected: selectedMembers[0]?.publicModelId } });
  ...
  private async filterCandidates(
    ...
    for (const c of candidates) {
      ...
      if (!upstream.enabled) {
        events.push({ step: 'filter_disabled', status: 'drop', details: { upstreamKeyId: upstream.id, reason: 'upstream disabled' } });
        continue;
      }
      if (upstream.frozen) {
        events.push({ step: 'filter_frozen', status: 'drop', details: { upstreamKeyId: upstream.id } });
        continue;
      }
      if (upstream.cooldownUntil && upstream.cooldownUntil > now) {
        events.push({ step: 'filter_cooldown', status: 'drop', details: { upstreamKeyId: upstream.id } });
        continue;
      }
      if (!providerSupportsProtocol(upstream.providerType, ir.sourceProtocol)) {
        events.push({ step: 'filter_protocol', status: 'drop', details: { upstreamKeyId: upstream.id, providerType: upstream.providerType } });
        continue;
      }
      if (!this.matchesCapabilities(upstream.providerType, required)) {
        events.push({ step: 'filter_capability', status: 'drop', details: { upstreamKeyId: upstream.id, required } });
        continue;
      }
        ...
        if (breaker) {
          if (breaker.state === 'open') {
            if (breaker.cooldownUntil && breaker.cooldownUntil > now) {
              events.push({ step: 'filter_breaker_open', status: 'drop', details: { upstreamKeyId: upstream.id } });
              continue;
            }
            // cooldown 已过，允许作为 half-open 尝试。
            events.push({ step: 'filter_breaker_half_open', status: 'ok', details: { upstreamKeyId: upstream.id } });
          }
        }
      }
      if (await this.quotaExhausted(upstream.id, now)) {
        events.push({ step: 'filter_quota', status: 'drop', details: { upstreamKeyId: upstream.id } });
        continue;
      }
      result.push(c);
    }
  ...
  private matchesCapabilities(providerType: UpstreamKeyRow['providerType'], required: RequiredCapabilities): boolean {
