...
describe('api client', () => {
  ...
  it('returns parsed JSON on success', async () => {
    mockFetch({
      ...
      text: async () => JSON.stringify({ id: 'pm_xxx', name: 'gpt-5' }),
    });

    const data = await api.get<{ id: string; name: string }>('/api/public-models');
    expect(data).toEqual({ id: 'pm_xxx', name: 'gpt-5' });
  });
