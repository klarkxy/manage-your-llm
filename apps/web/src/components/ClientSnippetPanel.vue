...
const modelOptions = computed(() =>
  publicModels.value.map((m) => ({ label: m.name, value: m.name })),
);
...
async function loadModels() {
  ...
  try {
    ...
    publicModels.value = models.map((m) => ({ id: m.id, name: m.name }));
    if (!selectedModel.value && publicModels.value.length > 0) {
      selectedModel.value = publicModels.value[0]!.name;
    }
  } catch {
